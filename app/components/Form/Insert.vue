...
} from '~/schemas/InsertSchema';
import type { InsertSchema } from '~/schemas/InsertSchema';
import {
...
const { insertPagePostType } = storeToRefs(uiStore);
...
function onTypeChange(newType: PostType) {
  uiStore.setInsertPagePostType(newType);
}

watch(insertPagePostType, (newType) => {
  state.type = newType;
});
  ...
  <UForm :state="state" @submit="onSubmit">
    <div class="auto-cols-auto grid-cols-2 grid-rows-1 lg:grid lg:gap-x-12 xl:gap-x-20">
      ...
      <div class="col-start-2 row-start-1 self-center">
        <!-- Type Selector -->
        <InputPostType v-model="state.type" name="type" @change="onTypeChange">
          <!-- Sale Amount -->
