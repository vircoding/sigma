...
const { formatAmount, formatFrequency } = usePost();
</script>
...
<template>
  ...
  >
    ...
    <span
      ...
      :class="[useStyles().textColorCardHero, useStyles().textSizeLG]"
      >{{ formatAmount(props.post.details.amount, true) }}</span
    >
  ...
  >
    ...
    <span
      class="relative top-0.5 mb-0.5 font-semibold leading-none md:leading-[22px]"
      :class="[useStyles().textColorCardHero, useStyles().textSizeLG]"
      >{{ formatAmount(props.post.details.tax, true) }}</span
    >
    ...
    <span
      class="relative top-0.5 leading-none md:leading-[22px]"
      :class="[useStyles().textColorCardHero, useStyles().textSizeSM]"
      >{{ props.post.details.currency }} / {{ formatFrequency(props.post.details.frequency) }}</span
    >
  </div>
