...
import { updateRentSchema } from '~/models/schemas/client/UpdatePostSchema';
import type { UpdateRentSchema } from '~/models/schemas/client/UpdatePostSchema';
import type { Map, Rent, UpdateRentInput } from '~/models/types/Post';
...
const state = reactive<UpdateRentInput>({
  tax: props.post.details.tax.toString(),
  currency: props.post.details.currency,
  frequency: props.post.details.frequency,
  phone: {
    phone: initPhone,
    code: initCode,
  },
...
const { handleSubmit, setFieldError, isSubmitting } = useForm<UpdateRentSchema>({
  validationSchema: toTypedSchema(updateRentSchema),
});

function onReset() {
  const { phone, code } = parsePhone(props.post.contact.phone);
  state.tax = props.post.details.tax.toString();
  state.currency = props.post.details.currency;
  state.frequency = props.post.details.frequency;
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    const data = await updateRent(values, props.id, map.value);
      ...
      <div class="col-start-2 row-start-1 self-center">
        <div class="flex">
          ...
          >
            ...
            <!-- Frequency -->
            <div class="flex flex-col">
              <InputPostFrequency v-model="state.frequency" name="frequency" />
            </div>
          </div>
        </div>
