...
const emit = defineEmits<{
  (e: 'crop', index: number, imageURL: string, file: Blob, absoluteIndex: number): void;
  (e: 'remove', index: number, absoluteIndex: number): void;
}>();
    ...
    result.canvas?.toBlob(
      (blob) => {
        if (blob) {
          ...
          emit('crop', props.index, croppedAvatarURL, blob, props.absoluteIndex);
          closeCropper();
        }
      },
      'image/jpeg',
    ...
    >
      ...
      >
        ...
        <ButtonIcon
          ...
          @click.stop="$emit('remove', props.index, props.absoluteIndex)"
        >
          <UIcon
