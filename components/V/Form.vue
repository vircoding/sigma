  ...
  <UForm :state="state" @submit="onSubmit">
    <div class="auto-cols-auto grid-cols-2 grid-rows-1 lg:grid lg:gap-x-12 xl:gap-x-20">
      ...
      <div class="col-start-2 row-start-1 self-center">
        <!-- Type Selector -->
        <VType v-model="state.type" name="type">
          <!-- Sale Amount -->
          ...
          <template #sale-currency>
            <VCurrency v-model="state.sale.currency" name="saleCurrency" />
          </template>
          ...
          <template #rent-currency>
            <VCurrency v-model="state.rent.currency" name="rentCurrency" />
          </template>

          <!-- Rent Frequency -->
          <template #rent-frequency>
            <VFrequency v-model="state.rent.frequency" name="rentFrequency" />
          </template>

          <!-- Exchange Offers -->
          <template #exchange-offers>
            <VOffers v-model="state.exchange.offers" name="exchangeOffers" />
          </template>

          <!-- Exchange Needs -->
          <template #exchange-needs>
            <VNeeds v-model="state.exchange.needs" name="exchangeNeeds" />
          </template>
        </VType>
    ...
    <div class="flex flex-col items-center justify-center gap-x-5 lg:flex-row lg:items-start">
      ...
      <div class="mb-4 w-full grow lg:mb-0">
        <VImage v-model="state.images" name="images" :error-visibility="errorVisibility" />
      </div>
